...
func SSRF(u *url.URL, hop int) *model.Risk {
	if util.IsInternalHost(u.Hostname()) {
		return &model.Risk{Type: "SSRF", Severity: "high", AtHop: hop, Detail: u.Host}
	}
	return nil
}

// HTTPSDowngrade reports if the scheme changed from https to http.
func HTTPSDowngrade(prev, next *url.URL, hop int) *model.Risk {
	if prev.Scheme == "https" && next.Scheme == "http" {
		return &model.Risk{Type: "HTTPS_DOWNGRADE", Severity: "medium", AtHop: hop, Detail: prev.String() + " -> " + next.String()}
	}
	return nil
}

// TokenLeakage detects sensitive tokens in query or fragment.
func TokenLeakage(u *url.URL, hop int) *model.Risk {
	q := u.Query()
	for k := range q {
		if tokenKeys[strings.ToLower(k)] {
			return &model.Risk{Type: "TOKEN_LEAK", Severity: "medium", AtHop: hop, Detail: k + " in query"}
		}
	}
	if frag := u.Fragment; frag != "" {
		for _, part := range strings.Split(frag, "&") {
			...
			if tokenKeys[strings.ToLower(kv[0])] {
				return &model.Risk{Type: "TOKEN_LEAK", Severity: "high", AtHop: hop, Detail: kv[0] + " in fragment"}
			}
		}
	}
	return nil
